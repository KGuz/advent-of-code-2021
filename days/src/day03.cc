...
auto to_bitset(const string &s) -> vector<bool> {
    return utils::map(s, [](auto c) { return static_cast<bool>(c - '0'); });
}

auto negate(const vector<bool> &bits) -> vector<bool> {
    return utils::map(bits, [](const auto bit) { return !bit; });
}

auto to_int(vector<bool> &bits) -> int {
...
auto most_common_bits(const vector<vector<bool>> &bitsets) -> vector<bool> {
    ...
    return utils::map(ones, [size = bitsets.size()](const auto &b) { return b >= size - b; });
}
...
auto reduce_common_bits(const vector<vector<bool>> &bitsets, Predicate pred) -> vector<bool> {
    ...
    for (auto n = 0; res.size() > 1; ++n) {
        ...
        auto filtered = res | views::filter([n, &common_bits](const auto &bits) { return bits[n] == common_bits[n]; });
...
auto Day03::part_one(string input) -> string {
    auto bitsets = utils::map(utils::lines(input), to_bitset);
...
auto Day03::part_two(string input) -> string {
    auto bitsets = utils::map(utils::lines(input), to_bitset);
